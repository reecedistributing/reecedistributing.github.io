  <template lang="pug">
    div
      | coool
      div#map
      //- script(src="", ref="script1", v-on:load="loaded[0] = true")
      //- script(src="", ref="script2", v-on:load="loaded[1] = true")
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      onLoad () {
        ...
        process.browser ? doMapStuff.apply(window) : null
      }
    },
    watch: {
      loaded: {
        deep: true,
        handler (value) {
          if(value[0] && value[1]) this.onLoad()
        }
      }
    },
    mounted () {
      if (process.browser) {
        [`http://d3js.org/d3.v4.min.js`, `http://d3js.org/topojson.v1.min.js`].forEach(
          (src,i) => {
            console.log(src)
  ...
  function doMapStuff () {
      const self = window 
      let scope = {}
      scope.originGeo = [44.0077, 116.9165];
      scope.originGeo = [16.8286, 52.4200];
      scope.originName = 'POZ';
      scope.destinations = [
        {coord: [45.7833, 108.5007]},
        {coord: [45.5231, 122.6765]}
      ]
      scope.destinations = [
      ...
      function calculateDuration(distance) {
        return (distance / scope.speed) * 1000;
      }
      function drawConnection(index) {
        debugger
        var destination = scope.destinations[index];
        debugger
        var originPos = scope.projection(scope.originGeo);
        var destinationPos = scope.projection(destination.coord);
        debugger
        var connection = [ originPos, destinationPos ];
        var destinationName = destination.name;
        var originGeo = scope.originGeo;
        var destinationGeo = destination.coord;
        var svg = scope.svg;
        debugger
        var distance = calculateDistance(scope.originGeo[1], scope.originGeo[0], destinationGeo[1], destinationGeo[0]);
      ...
      function drawMap(originName, originGeo, destinations) {
        debugger
        let self = window;
        var countries, height, path, 
        // projection,  // GLOBALIZING PROJECTION
        scale, svg, width;
        var width = 800;
        var height = 800;
        var center = [4, 68.6];
        var scale = 700;
        scope.projection = d3.geoMercator().scale(scale).translate([width / 2, 0]).center(center);
        let bubblegum = 3
        debugger
        path = d3.geoPath().projection(scope.projection);
        svg = d3.select('#map').append('svg')
          .attr('height', height)
          .attr('width', width)
          .style('background', '#C1E1EC');
        countries = svg.append("g");
        let json_data = `https://fortress.wa.gov/dnr/arcgisext/weba_ext_prod1/rest/services/OpenData/WADNR_Natural_Heritage_Open_Data/MapServer/0/query?where=1%3D1&outFields=*&outSR=4326&f=json`
        json_data = 'https://raw.githubusercontent.com/jesion/d3js-flight-connections-map/master/src/europe.json'
        ...
        var source = svg.selectAll('circleOrigin');
          source
          .data([scope.originGeo]).enter()
          .append('circle')
          .attr('cx', function (d) { return scope.projection(d)[0]; })
          .attr('cy', function (d) { return scope.projection(d)[1]; })
          .attr('r', '3px')
          .style('opacity', 1)
          .attr('fill', 'green')
          ...
          .on('mouseover', function (d) {
            tooltip.html('<span style="color:white">' + originName + '</span>')
              ...
              .style('left', scope.projection(d)[0] + 12 + 'px')
              .style('top', scope.projection(d)[1] - 20 + 'px')
              .transition()
              .duration(700)
              .style('opacity', 1)
          })
        ...
        scope.originGeo = scope.originGeo
        drawConnections();
      };
      drawMap(scope.originName, scope.originGeo, scope.destinations);
  }
  </script>
